...
/*   Updated: 2025/07/19 15:43:13 by rookuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	export_loop(char **args, t_envp_data *envp_data)
{
	int	i;
	int	check_num;
...
static void	write_export_result(t_envp_data *envp_data, char *key, int i)
{
	write(1, "declare -x ", 11);
	write(1, key, ft_strlen(key));
	write(1, "=\"", 2);
	write(1, &envp_data->envp[i][ft_strlen(key) + 1],
		ft_strlen(&envp_data->envp[i][ft_strlen(key) + 1]));
	write(1, "\"\n", 2);
}
		...
		*envp_data = envp_line_up(*envp_data);
		while (envp_data->envp[i] != NULL)
		{
			if (ft_strncmp(envp_data->envp[i], "_=", 2) != 0)
			{
				...
				write_export_result(envp_data, key, i);
				free(key);
			}
			i++;
		}
